...
DECLARE i RECORD;
BEGIN DROP TABLE IF EXISTS temp_categoria_product;
...
EXECUTE format(
	'COPY temp_categoria_product (id, category, name_product, price, reference_price, reference_unit, date_product)
FROM %L DELIMITER '','' CSV HEADER;',
	path_csv
);
FOR i IN
SELECT DISTINCT category
FROM temp_categoria_product LOOP IF (
		SELECT 1
		FROM negocio.categoria_producto
		WHERE categoria_producto.nombre = i.category
	) = 1 THEN RAISE NOTICE 'La categoria ya existe %',
	i.category;
ELSE
