...
---------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE negocio.importar_catalogo_productos(path_csv varchar)
LANGUAGE plpgsql
AS $$
DECLARE
    i RECORD;
    ...
    FOR i IN (
        ...
        LOOP
            ...
            IF (
                ...
                WHERE
                    categoria_producto.nombre = i.category) IS NULL THEN
            ...
            ELSE
                INSERT INTO negocio.producto(codigo, nombre, id_categoria, descripcion, precio, ultimo_cambio)
                    VALUES (i.id, i.name_product,(
                            SELECT
                                categoria_producto.id_categoria
                            FROM
                                negocio.categoria_producto
                            WHERE
                                categoria_producto.nombre = i.category), NULL, CAST(REPLACE((REPLACE(REPLACE(i.reference_price, '$', ''), ' ', '')), ',', '.') AS DECIMAL(10, 2)), NOW());
            END IF;
        END LOOP;
END
$$;
