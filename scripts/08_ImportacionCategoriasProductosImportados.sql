...
---------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE negocio.importar_categorias_productos_importados(path_csv varchar)
LANGUAGE plpgsql
AS $$
DECLARE
    i RECORD;
...
FROM %L DELIMITER '';'' CSV HEADER;', path_csv);
    FOR i IN ( SELECT DISTINCT
            categoria
        FROM
            temp_categoria_producto_importado)
        LOOP
            IF (
                SELECT
                    1
                ...
                WHERE
                    categoria_producto.nombre = i.categoria AND categoria_producto.id_clasificacion IS NOT NULL) = 1 THEN
                RAISE NOTICE 'La categoria ya existe %', i.categoria;
            ELSIF (
                    SELECT
                        1
                    FROM
                        negocio.categoria_producto
                    WHERE
                        categoria_producto.nombre = i.categoria
                        AND categoria_producto.id_clasificacion IS NULL) = 1 THEN
                ...
                SET
                    id_clasificacion =(
                        ...
                        WHERE
                            clasfificacion_categoria_producto.nombre = 'Importados')
                WHERE
                    categoria_producto.nombre = i.categoria
                    AND categoria_producto.id_clasificacion IS NULL;
            ELSE
