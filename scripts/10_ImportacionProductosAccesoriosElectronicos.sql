...
---------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE negocio.importar_catalogo_productos_acc_electronicos(path_csv varchar)
LANGUAGE plpgsql
AS $$
DECLARE
    i RECORD;
        ...
        LOOP
            INSERT INTO negocio.producto(nombre, id_categoria, descripcion, precio, ultimo_cambio)
                VALUES (i.product,(
                        ...
                        WHERE
                            categoria_producto.nombre = 'Accesorios Electronicos'), NULL, CAST(REPLACE((REPLACE(REPLACE(i.precio_unit_usd, '$', ''), ' ', '')), ',', '.') AS DECIMAL(10, 2)), NOW());
        END LOOP;
END
$$;
