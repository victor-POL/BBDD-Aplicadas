...
EXECUTE format(
    'COPY temp_clasificacion_producto (linea_producto, producto)
FROM %L DELIMITER '';'' CSV HEADER;',
    path_csv
);
FOR i IN
SELECT DISTINCT linea_producto
FROM temp_clasificacion_producto LOOP IF (
        SELECT 1
        FROM negocio.clasfificacion_categoria_producto
        WHERE clasfificacion_categoria_producto.nombre = i.linea_producto
    ) = 1 THEN RAISE NOTICE 'La clasificacion ya existe %',
    i.linea_producto;
ELSE
